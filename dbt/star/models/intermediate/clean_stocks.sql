...
cleaned_data as (
    select
        ...
        safe_cast(replace(split(bid, ' x ')[safe_offset(1)], ',', '') as float64) as bid_size,
        safe_cast(split(ask, ' x ')[safe_offset(0)] as float64) as ask_price,
        safe_cast(replace(split(ask, ' x ')[safe_offset(1)], ',', '') as float64) as ask_size,
        ...
        case
            when intraday_market_cap like '%t' then safe_cast(replace(intraday_market_cap, 't', '') as float64) * 1e12
            when intraday_market_cap like '%b' then safe_cast(replace(intraday_market_cap, 'b', '') as float64) * 1e9
            when intraday_market_cap like '%m' then safe_cast(replace(intraday_market_cap, 'm', '') as float64) * 1e6
            when intraday_market_cap like '%k' then safe_cast(replace(intraday_market_cap, 'k', '') as float64) * 1e3
        ...
        safe_cast(split(forward_dividend_and_yield, ' ')[safe_offset(0)] as float64) as forward_dividend_payout,
        safe_cast(regexp_replace(split(forward_dividend_and_yield, ' ')[safe_offset(1)], r'[()%]', '') as float64) / 100 as forward_dividend_yield,
...
final as (
    select
        {{ dbt_utils.generate_surrogate_key(['company_symbol', 'load_date']) }} as pk_ticker_date,
        *
    from cleaned_data
)
