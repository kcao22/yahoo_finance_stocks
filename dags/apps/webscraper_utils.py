...
from apps.data_source_utils.yahoo_finance_config import (
...
class YahooFinanceScraper(WebScraper):
    ...
    async def scrape_company_data(
        self, company_symbol: str, extract_config: list[dict], max_retries: int = 5
    ) -> list[dict]:
        """
        ...
        retry_fields = (
            ["open", "previous_close", "volume", "avg_volume", "day_range"]
            if extract_config == DAILY_EXTRACT_CONFIG
            else ["company_sector", "company_industry", "company_full_time_employees"]
        )
        for attempt in range(max_retries):
            ...
            try:
                ...
                print(
                    f"(Attempt {attempt + 1}/{max_retries}) for company {company_symbol}..."
                )
                print(f"Navigating to {extract_url}...")
                # Proceed once basic HTML loads
                await page.goto(
                    extract_url, wait_until="domcontentloaded", timeout=60000
                )
                await asyncio.sleep(
                    random.uniform(1, 3)
                )  # Buffer to allow potential re-routing from invalid symbol
                ...
                if extract_config != DIM_DATA_EXTRACT_CONFIG:
                    expand_button = page.locator(
                        'button[data-testid="accordionItem"]:has-text("Overview")'
                    )
                ...
                missing_fields = [
                    ...
                    if data.get(field) in [None, "N/A", ""]
                ]
                if missing_fields and attempt < max_retries - 1:
                    print(
                        f"Missing crucial fields {missing_fields} for {company_symbol}. Retrying..."
                    )
                ...
                print(
                    f"Error scraping data for {company_symbol} on attempt {attempt + 1}: {e}"
                )
                if page:
                    await page.close()
    ...
    async def scrape_companies_data(
        self,
        company_symbols: list[str],
        stock_or_profile: str,
        ...
        if stock_or_profile not in ["stock", "profile"]:
            raise ValueError(
                f"Invalid stock_or_profile value: {stock_or_profile}. Must be 'stock' or 'profile'."
            )
        ...
        for i in range(0, len(company_symbols), max_concurrency):
            ...
            tasks = [
                self.sem_task(symbol, semaphore, extract_config) for symbol in batch
            ]
    ...
    async def sem_task(self, symbol, semaphore, extract_config):
        async with semaphore:
            return await self.scrape_company_data(
                company_symbol=symbol, extract_config=extract_config
            )
